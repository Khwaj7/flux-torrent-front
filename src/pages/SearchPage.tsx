...
import { Loader2, Magnet, Search, Frown } from "lucide-react";
...
import { useTorrentSearch } from "../hooks/useTorrentSearch.ts"; // <-- Import du hook

export default function SearchPage() {
  const { data: torrents, isLoading, error } = useTorrents();

  // Une seule ligne pour toute la logique !
    ...
    <div className="container pt-10 pb-20">
      ...
      <section className="text-center mb-16">
        ...
        <div className="flex justify-center gap-3 flex-wrap">
          {["Tout", "Audio", "Vidéo", "Apps", "Jeux", "Adulte"].map(
            (filter, idx) => (
              <button
                key={idx}
      ...
      <main className="bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] rounded-3xl p-8 shadow-2xl overflow-x-auto min-h-[400px]">
        {search.isEmpty ? (
          <div className="flex flex-col items-center justify-center py-20 text-[var(--text-muted)] animate-[fadeIn_0.5s]">
          ...
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="text-sm">
              {/* Utilisation de search.results */}
              {search.results.map((torrent) => {
                const style =
                  TYPE_STYLES[torrent.type] || TYPE_STYLES["Default"];
