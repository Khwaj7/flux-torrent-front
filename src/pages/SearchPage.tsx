...
export default function SearchPage() {
  // Typage explicite du tableau de torrents
  const torrents: Torrent[] = [
    { id: 1, name: 'Big Buck Bunny 4K Remaster', size: '2.4 GB', uploader: 'BlenderFdn', date: 'Auj.', se: 4205, le: 12, type: 'Vidéo' },
    { id: 2, name: 'Ubuntu 24.04 LTS Desktop', size: '4.8 GB', uploader: 'Canonical', date: 'Hier', se: 8502, le: 340, type: 'App' },
    { id: 3, name: 'Arch Linux Core ISO 2025', size: '800 MB', uploader: 'ArchUser', date: '01-01', se: 15000, le: 200, type: 'App' },
  ];
    ...
    <div className="container pt-10 pb-20">
      <header className="flex justify-between items-center p-6 bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] rounded-3xl mb-12 shadow-2xl">
        <div className="text-2xl font-bold tracking-widest flex items-center gap-2">
           <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--primary-accent)]">
      ...
      <section className="text-center mb-16">
        ...
        <div className="relative max-w-2xl mx-auto mb-8">
          ...
          />
          <button className="absolute right-3 top-1/2 -translate-y-1/2 w-12 h-12 bg-[var(--primary-accent)] rounded-full flex items-center justify-center hover:scale-110 transition text-black">
        ...
        <div className="flex justify-center gap-3 flex-wrap">
          {['Tout', 'Audio', 'Vidéo', 'Apps', 'Jeux'].map((filter, idx) => (
            <button key={idx} className={`px-6 py-2 rounded-full border border-[var(--glass-border)] transition ${idx === 0 ? 'bg-[var(--glass-highlight)] border-[var(--primary-accent)]' : 'hover:bg-white/10'}`}>
              {filter}
            </button>
          ))}
        </div>
      ...
      <main className="bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] rounded-3xl p-8 shadow-2xl overflow-x-auto">
            ...
            {torrents.map((torrent) => (
              <tr key={torrent.id} className="group hover:bg-white/5 transition rounded-xl">
                <td className="py-4 pl-4 border-b border-white/5 group-last:border-none">
                  <Link to={`/details/${torrent.id}`} className="block text-base font-semibold text-white mb-1 hover:text-[var(--primary-accent)] transition">
                    {torrent.name}
                  </Link>
                  <div className="flex gap-2 items-center text-xs text-[var(--text-muted)]">
                    ...
                    <span>{torrent.size}</span> • <span>Par <strong className="text-gray-300">{torrent.uploader}</strong></span>
                  </div>
                </td>
                ...
                <td className="py-4 border-b border-white/5">
                  <button className="text-[var(--primary-accent)] border border-[var(--primary-accent)] px-3 py-1.5 rounded-lg text-xs hover:bg-[var(--primary-accent)] hover:text-black transition flex items-center gap-1">
